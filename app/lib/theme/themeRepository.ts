...
import { blendHex, normalizeHexColor } from "~/lib/utils/color";
import { DISPLAY_PALETTE_KEYS, type ResolvedTheme, type ThemeColorKey, type ThemeOption } from "./types";
...
function normalizeTheme(themeInput: unknown): ResolvedTheme {
  ...
  const name =
    String(getPathValue(theme, "name") ?? "").trim() || fallbackTheme.name;
  const description =
...
function extractThemeObjectMap(value: unknown): Record<string, unknown> {
  if (!isObject(value)) return {};
  if (isObject((value as ThemeObject).themes)) {
    return (value as ThemeObject).themes as Record<string, unknown>;
  }
  return value as Record<string, unknown>;
}

const rawThemes = {
  ...extractThemeObjectMap(themesManifest),
  ...extractThemeObjectMap(additionalThemesManifest),
};
...
export const themeNames = Object.keys(themesByName);
